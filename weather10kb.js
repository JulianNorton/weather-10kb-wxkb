...
function Weather10kbRequest(request) {
  this.geocode = () => new Promise((resolve, reject) => {
    if (typeof request.params.location === 'string') {
      ...
      nodeFreegeoip.getLocation(ip, (err, location) => {
        ...
        return resolve(location.region_code);
      });
    }
  });

  this.setTimeZone = () => new Promise((resolve, reject) => {
  ...
  this.getForecast = () => new Promise((resolve, reject) => {
...
router.get('/:location?', (request, response) => {
  ...
  .then((data) => {
    ...
    if (request.params.longitude === 0 && request.params.latitude === 0) {
      throw new Error('Undetermined location.');
    }

    const args = objectMerge(data, { params: request.params });
